...
// Search from the end and find the element with the given condition
const withdrawalIndex = movements.findLastIndex(mov => mov < 0); // returns index
const withdrawalElement = movements.findLast(mov => mov < 0); // returns the element itself

const biggestLargeMovements = movements.findLast(mov => Math.abs(mov) > 2000);
const biggestLargeMovementsIndex = movements.findLastIndex(
  mov => Math.abs(mov) > 2000
);
console.log(
  `You last Largest withDrawal: ${biggestLargeMovements} and it was ${
    movements.length - biggestLargeMovementsIndex
  } ago`
);
