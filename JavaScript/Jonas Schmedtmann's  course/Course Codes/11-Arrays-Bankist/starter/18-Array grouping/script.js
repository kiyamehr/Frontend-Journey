...
const groupedByMovements = Object.groupBy(movements, movement =>
  movement > 0 ? 'Deposit' : 'Withdraw'
);

const groupedByActivity = Object.groupBy(accounts, account => {
  const lengthMovements = account.movements.length;

  if (lengthMovements >= 8) return 'very active'; // key name
  if (lengthMovements >= 4) return 'active'; // key name
  if (lengthMovements >= 1) return 'moderate'; // key name
  return 'inactive'; // else:
});
console.log(groupedByActivity);

// Returned type will be the key
